...
{
	@$connect = mysqli_connect("localhost","root","","charity") or die("couldn't connect!");
	@$query3 = mysqli_query($connect , "SELECT `email` FROM `ngo_details` WHERE `email`='$Email' AND `Permission`='1'");
	@$numrows3 = mysqli_num_rows($query3);

	if(@$numrows3 != 0)
	{
		@$query3 = mysqli_query($connect , "UPDATE `ngo_details` SET `password` = '$npsw' WHERE `ngo_details`.`email` = '$Email' AND `ngo_details`.`Permission` = '1'");
		echo '<script type="text/javascript">
				alert("Password Reset Sucessfully!");
